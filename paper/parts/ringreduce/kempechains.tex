...
Judging only from the colors of a ring such as in $abcab$, we have no information on the chains that are present. This information is important because two vertices of the ring that belong to the same chain must be flipped together. To include this information in a ring coloring, Birkhoff devised the notion of a \textit{scheme}. He simply draws a line between two colors on the ring if they belong to the same Kempe-chain.
